...
struct TaskTemplate { uint32 id, killEntry, killCount, rewardGold, rewardItemId, rewardItemCount, minLevel, maxLevel; };
struct TaskProgress { uint32 taskId, currentCount, targetCount; bool completed; };
...
void TaskBoard::Load() { LOG_INFO("module", ">> mod-mortal: TaskBoard system loaded"); LoadTaskTemplates(); }
void TaskBoard::Unload() { s_taskTemplates.clear(); s_playerTasks.clear(); }
...
void TaskBoard::OnCreatureDeath(Creature* creature, Unit* killer)
{
    ...
    if (playerIt == s_playerTasks.end()) return;
    ...
    for (auto& taskPair : playerIt->second)
    {
        ...
        if (templateIt == s_taskTemplates.end() || templateIt->second.killEntry != creature->GetEntry()) continue;
...
bool TaskBoard::OfferTask(Player* player, uint32 taskId)
{
    ...
    if (it == s_taskTemplates.end()) return false;
    s_playerTasks[player->GetGUID().GetCounter()][taskId] = { taskId, 0, it->second.killCount, false };
    ChatHandler(player->GetSession()).PSendSysMessage("Task accepted: Kill %u creatures", it->second.killCount);
    return true;
}
...
bool TaskBoard::CompleteTask(Player* player, uint32 taskId)
{
    ...
    if (playerIt == s_playerTasks.end()) return false;
    auto taskIt = playerIt->second.find(taskId);
    if (taskIt == playerIt->second.end() || !taskIt->second.completed) return false;
    auto templateIt = s_taskTemplates.find(taskId);
    if (templateIt == s_taskTemplates.end()) return false;

    if (templateIt->second.rewardGold > 0) player->ModifyMoney(templateIt->second.rewardGold * GOLD);
...
{ auto it = s_playerTasks.find(player->GetGUID().GetCounter()); return it != s_playerTasks.end() && it->second.find(taskId) != it->second.end(); }

uint32 TaskBoard::GetTaskProgress(Player* player, uint32 taskId)
{ auto it = s_playerTasks.find(player->GetGUID().GetCounter()); return it != s_playerTasks.end() && it->second.find(taskId) != it->second.end() ? it->second[taskId].currentCount : 0; }

void TaskBoard::SetTaskProgress(Player* player, uint32 taskId, uint32 progress)
{ s_playerTasks[player->GetGUID().GetCounter()][taskId].currentCount = progress; }

void TaskBoard::GenerateDailyTasks(Player* player) {}
uint32 TaskBoard::GetAvailableTaskCount(Player* player) { return static_cast<uint32>(s_taskTemplates.size()); }
