...
void RegionalBank::Load() { LOG_INFO("module", ">> mod-mortal: RegionalBank system loaded"); LoadBankNpcs(); }
void RegionalBank::Unload() { s_zoneBankMapping.clear(); s_bankNpcEntries.clear(); }
...
{
    if (count == 0 || slot > 100) return false;
    uint32 zone = GetAssociatedZone(zoneId) ? GetAssociatedZone(zoneId) : zoneId;
    uint32 guid = player->GetGUID().GetCounter();
    CharacterDatabasePreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_MORTAL_REGIONAL_BANK_ITEM);
    stmt->SetData(0, guid); stmt->SetData(1, zone); stmt->SetData(2, slot);
    stmt->SetData(3, 0); stmt->SetData(4, itemEntry); stmt->SetData(5, count);
...
bool RegionalBank::WithdrawItem(Player* player, uint32 zoneId, uint8 slot, uint32 count)
{
    ...
    stmt->SetData(0, guid); stmt->SetData(1, zone);
    ...
    do {
        ...
        delStmt->SetData(0, guid); delStmt->SetData(1, zone); delStmt->SetData(2, slot);
...
uint32 RegionalBank::GetItemCount(Player* player, uint32 zoneId, uint8 slot)
{
    ...
    stmt->SetData(0, guid); stmt->SetData(1, zone);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);
    if (result) do { Field* fields = result->Fetch(); if (fields[0].Get<uint8>() == slot) return fields[1].Get<uint32>(); } while (result->NextRow());
    return 0;
}

uint32 RegionalBank::GetAssociatedZone(uint32 areaId) { auto it = s_zoneBankMapping.find(areaId); return it != s_zoneBankMapping.end() ? it->second : 0; }
bool RegionalBank::IsRegionalBankNpc(uint32 entry) { return std::find(s_bankNpcEntries.begin(), s_bankNpcEntries.end(), entry) != s_bankNpcEntries.end(); }

void RegionalBank::LoadBankNpcs()
{
    s_bankNpcEntries = { 2500, 2501, 2502 };
    s_zoneBankMapping[1] = 1; s_zoneBankMapping[3] = 3; s_zoneBankMapping[1519] = 1519;
}
